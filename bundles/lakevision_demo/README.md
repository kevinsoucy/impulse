# LakeVision demo bundle (ADAS data domain)

Databricks Asset Bundle for the Impulse + LakeVision BYOD demo. Two structural choices vs. the older `byod_demo` bundle:
...
`catalog` is required and has no default — workspaces vary on what catalogs are provisioned. Other defaults: `schema_prefix=lakevision_demo`, `adapter=nuscenes`, `dataset_version=v1.0-mini`. Override any of these with additional `--var "name=value"` flags.

> **Always pass `--target shared`.** The bundle's default `dev` target is DAB's development mode — it prefixes every workspace path with `${workspace.current_user.userName}` and every schema name with `dev_<username>_`. The schema-name prefix collides with `BYODConfig`'s bare-name table assembly; deploys land but the notebooks then fail to find their tables. `--target shared` writes to `/Workspace/Shared/.bundle/lakevision_demo/...` so multiple SAs on the same workspace see the same bundle. The "Interactive notebook use" section below depends on this path.
...
2. Uploads both to `/Workspace/Shared/.bundle/lakevision_demo/artifacts/.internal/`.
3. Registers two jobs:
   - `adas — bootstrap LakeVision tables + ingest metadata`
   - `adas — end-to-end (ingest → detect → per-event detail → visualize)`
4. Syncs notebooks (`demos/byod/*.py`), `lib/`, `adapters/`, and this `notebook_env.yml` to `/Workspace/Shared/.bundle/lakevision_demo/files/`.

No `databricks fs cp` step. No manual wheel handling.

**Schemas and volumes are not bundle-managed.** They are created on first run by the first cell of `01_ingest.py` via `CREATE SCHEMA IF NOT EXISTS` + `CREATE VOLUME IF NOT EXISTS`. Schemas: `<schema_prefix>_silver`, `<schema_prefix>_perception_silver`, `<schema_prefix>_gold`. Volumes (under `<schema_prefix>_silver`): `raw`, `camera_frames`, `lidar_scans`, `openlabel_packages`. This separation is intentional — see CHANGELOG BL-015 for the rationale (bundle-managed UC resources are subject to destructive Terraform recreates on state disturbance, which would nuke staged data).

---

## First run — bootstrap, then stage, then run_all_job

The bundle does not download datasets (registration-gated for most, signed-URL-expiring for NuScenes), and as of the BL-015 change the schemas + volumes are not bundle-managed either. The first cell of `01_ingest.py` provisions them, so the order is:

```bash
# 1. Provision schemas, volumes, and empty LakeVision tables, then write metadata.
...
databricks fs cp -r \
  ...
  /Volumes/<catalog>/lakevision_demo_silver/raw/v1.0-mini \
  --profile <your-workspace-profile>
...
`bootstrap_job` runs `01_ingest.py` with `bootstrap_only=true`: provisions the three schemas + four volumes (idempotent `CREATE … IF NOT EXISTS`), creates the empty LakeVision tables, writes metadata via the selected adapter, then exits. Re-runnable idempotently as a recovery / table-reset step.
...
Quick SQL spot-checks after `run_all_job` completes. Run via DBSQL or `databricks sql query --warehouse-id <id>`. Schemas resolve from `--var "catalog=<your-catalog>"` plus the default `schema_prefix=lakevision_demo`.

The block below assumes the defaults (`adapter=nuscenes`, `schema_prefix=lakevision_demo`). If you overrode either at deploy time, substitute the table prefix accordingly — Impulse persists event tables under `<adapter>_demo_` in the gold schema (`mda_reporting`'s `UnitySink` requires a `table_prefix`).

```sql
-- t01_ingest cross-table invariant (the channel_metrics index event search inner-joins on)
SELECT
  (SELECT COUNT(*) FROM lakevision_demo_silver.channel_metrics)                     AS metrics_rows,
  (SELECT COUNT(*) FROM (SELECT DISTINCT container_id, channel_id
                         FROM lakevision_demo_silver.channels))                     AS distinct_pairs;
-- expect:  metrics_rows == distinct_pairs

-- t02_detect_events — events + playlist windows
SELECT COUNT(*) AS n_events  FROM lakevision_demo_gold.nuscenes_demo_event_instance_fact;
SELECT COUNT(*) AS n_windows FROM lakevision_demo_perception_silver.playlist_items;
-- expect for NuScenes v1.0-mini: n_events = 9 (one per scene with a pedestrian-close-at-speed
-- match), n_windows = 9. Larger variants and other adapters yield more.

-- t03_per_event_detail — one OpenLABEL JSON per playlist window
LIST '/Volumes/<catalog>/lakevision_demo_silver/openlabel_packages/pedestrian_high_speed_v1/';
-- expect for NuScenes v1.0-mini: 9 .json files (= n_windows above).
```
...
When you open a deployed notebook in the workspace UI, the job's environment spec is **not** applied — interactive runs get the bare serverless default and the first cell fails with `ModuleNotFoundError: mda_reporting`. To fix:

1. Open the notebook (e.g., `/Workspace/Shared/.bundle/lakevision_demo/files/demos/byod/02_detect_events`).
...
> **Path assumes `--target shared`.** The absolute paths in steps 1 and 4 are only valid after a `--target shared` deploy. If you deployed with the `dev` default, both files live under `/Workspace/Users/${workspace.current_user.userName}/.bundle/...` and copy-pasting the paths above into the Environment side panel will fail silently (the file picker just won't find them). Re-deploy with `--target shared` per the warning under "First-time deploy".
...
- `bundles/lakevision_demo/resources/jobs.yml` (both jobs' `environments.serverless.dependencies` blocks)
- `bundles/lakevision_demo/notebook_env.yml` (the `dependencies:` list)
...
Destroys the two jobs, the DAB-managed artifacts at `${workspace.root_path}/artifacts/.internal/`, and the workspace files at `${workspace.root_path}/files/`. **Schemas, volumes, and all staged data survive** — they are not bundle-managed (see BL-015 in CHANGELOG for the rationale). This is deliberate: `bundle destroy` is now safe to run as a cleanup of code/artifacts without losing the dataset.

If you want a full reset, run these by hand after `bundle destroy`:

```sql
DROP SCHEMA IF EXISTS <catalog>.lakevision_demo_silver CASCADE;
DROP SCHEMA IF EXISTS <catalog>.lakevision_demo_perception_silver CASCADE;
DROP SCHEMA IF EXISTS <catalog>.lakevision_demo_gold CASCADE;
```
...
Schemas and volumes are intentionally not in `resources/` — they're created by `01_ingest.py` so a destructive Terraform recreate can't propose to nuke them. See CHANGELOG BL-015.
