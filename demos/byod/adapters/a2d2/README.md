...
The bundle dependency list in `resources/jobs.yml` does not include any A2D2-specific package — the adapter uses only `numpy`, Pillow, and the stdlib (`json`, `pathlib`). The `nuscenes-devkit` dependency is unused when `adapter=a2d2` but is harmless to leave installed.
...
**Detection aggregates** (per ADR-2, derived from per-frame annotations):
...
## Unpopulated tables (per the BL-003 spec)
