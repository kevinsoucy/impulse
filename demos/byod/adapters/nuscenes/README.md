...
[NuScenes](https://www.nuscenes.org/) plugged into the BYOD demo. The reference adapter — NuScenes has no CAN bus, so `scalar_source()` synthesizes 9 scalars per scene from ego pose + annotations. Notebooks 03–09 are byte-identical to every other adapter; only 01–02 dispatch through here.
...
See `bundles/lakevision_demo/README.md` for the full prerequisites + first-time-deploy walkthrough.

The bundle's `resources/jobs.yml` pins `nuscenes-devkit==1.1.9` (the 1.1.11 release pins `matplotlib<3.6` which is incompatible with the numpy-2.x stack on serverless env v4).
...
| Phase 4 | `camera_object_detections` | Same annotations projected per camera via `nuscenes-devkit` calibration |
...
Each derived channel carries `synthesized=true` and `derivation` tags in `channel_tags` (ADR-2 enrichment).
...
| `config.yaml` | `dataset_versions`, `dataroot_template`, `nuscenes-devkit` dep. |
| `loader.py` | Wraps `nuscenes-devkit` NuScenes API; `Scene` dataclass. |
...
| `camera.py` | 3D box → `camera_object_detections` via devkit calibration. |
