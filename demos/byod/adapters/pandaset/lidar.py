...
**BL-009 differentiator:** each cuboid produces TWO rows, one per physical
LiDAR sensor (`LIDAR_SPINNING` Pandar64, `LIDAR_SOLIDSTATE` PandarGT). This
gives notebook 09's `distance_stats_by_modality` a within-adapter sensor
...
Per ADR-7, this table is populated only inside TSAL event windows.
"""
...
def map_annotation_to_lidar_detections(ann: Annotation) -> list[dict]:
    ...
    fused across both LiDARs). To match the BL-009 acceptance criterion
    ("notebook 09 shows distinct range distributions for the two sensors")
    we duplicate each cuboid into two rows tagged with the per-physical-sensor
