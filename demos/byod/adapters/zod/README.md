...
ZOD's adapter requires `h5py>=3.0` (declared in `config.yaml`). The `nuscenes-devkit` pin in the bundle's `resources/jobs.yml` is unused but harmless when `adapter=zod`.
...
Each derived channel carries an `oxts_path` tag in `channel_tags` (ADR-2 enrichment) pointing back to the source HDF5 path.
...
**Notebook 09's `distance_stats_by_modality` will show non-empty `radar` rows only with `adapter=zod`** — this is the BL-008 differentiator.
...
|---|---|
| `radar_object_detections` | Per-detection radar emission is a planned ZOD-specific extension. Doppler is captured per annotation in `Annotation.radar_doppler_ms` (loader.py); a future notebook can lift these into a dedicated radar table. See BL-008 ticket findings. |
