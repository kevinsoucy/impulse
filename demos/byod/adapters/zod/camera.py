"""Convert ZOD annotations to `camera_object_detections` rows.

ZOD ships one primary camera (`camera_front_blur`) per sequence with full
extrinsics + intrinsics in `calibration.json`. The projection chain is the
same as A2D2's — vehicle-frame box → cam frame → image plane via the
intrinsic matrix — so this module is a thin wrapper.
...
def _box_corners_vehicle(
    ...
    """8 corner positions in the vehicle frame. ZOD's size order is
    (length, width, height) — same as A2D2 and the LakeVision schema."""
    l, w, h = size_lwh
