...
ZOD-specific properties vs A2D2:
  ...
  - Each annotation has separate `lidar_attribute.num_points` and
    ...
    notebook 09's `distance_stats_by_modality` partitions on. **This is the
    BL-008 differentiator** — ZOD is the only adapter where the radar modality
    populates non-empty in notebook 09's KPI tables.
