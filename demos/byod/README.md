...
| **Shared demo helpers** (`lib/`) | `geometry`, `windowing`, `openlabel`, `scalar_metrics`, `silver_schema`; core channel schema from `impulse_query_engine.schema` | Don't edit |
...
python_deps: []            # extra pip deps for this adapter, if any
```
...
| 02 | `02_detect_events.py` | Find | `channels` | `event_instance_fact`, `event_dimension` |
| 03 | `03_per_event_detail.py` | Package | detected event windows + per-window adapter mappings | `lidar_object_detections`, `camera_object_detections`, OpenLABEL JSON packages |
| 04 | `04_visualize.py` | Show | event windows + perception_channels + object_tracks | Event walkthrough figures + sensor KPI tables/histograms |
