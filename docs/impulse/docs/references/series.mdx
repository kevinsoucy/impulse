...
Impulse 1.0 held exactly one shape — a scalar channel, one value per
...
**Impulse 1.0 — one shape**

<div className="max-w-lg my-4">
  <ShapeTable
    badge="1.0"
    title="Scalar channel (RLE)"
    caption="One value per (channel, interval). The only shape 1.0 can hold."
    columns={[
...
**Impulse 2.0 — your tables, any shape**
...
<div className="grid gap-4 lg:grid-cols-2 my-4">
  ...
  <ShapeTable
    ...
    highlight={{
      rows: [0, 1, 2],
      col: 2,
      note: 'Three objects share one frame_ts — three rows for a single instant. A 1.0 scalar channel holds exactly one value per (signal, time), so this shape is unrepresentable in 1.0.',
    }}
  />
...
|-----------------|-------------|
| A single **numeric** value per `(signal, instant)` | Scalar channel (one per signal) — the 1.0 case |
