...
- Library helper outputs match the schema declared in `src/lakevision/schema/`.
...
uv run --exact --all-extras pytest tests/lakevision/integration/ -v
```

Single file:

```
uv run --exact --all-extras pytest tests/lakevision/integration/test_nuscenes_pipeline.py -v
```
