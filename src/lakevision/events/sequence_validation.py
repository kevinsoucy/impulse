...
``PerceptionMeasurementDB``-backed ``Report``.

Core ``SequenceOfEvents`` is unchanged — sequences over scalar predicates
work exactly as before. The LakeVision concern is: when a sequence has
perception steps, the steps must agree on ``track_scope``. A sequence that
...
from lakevision.tsal.perception_selector import (
    PerceptionSelector,
