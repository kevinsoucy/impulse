...
The shared integration tests under `tests/lakevision/integration/` exercise every registered adapter through the BYOD pipeline. Once your adapter is registered (`lib.adapter.register(...)` in your `__init__.py`), those tests pick it up automatically.
